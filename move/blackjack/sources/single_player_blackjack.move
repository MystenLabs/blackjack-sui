...
entry fun stand(
    ...
    } else //case dealer got blackjack and player got twenty-one
    //dealer wins
    if (
        game.dealer_sum == 21
                 && game.player_sum == 21
...
#[test_only]
public fun draw_card_for_testing(game: &mut Game, is_dealer: bool, card_idx: u8) {
    if (!is_dealer) {
...
#[test_only]
public fun pop_card_for_testing(game: &mut Game, is_dealer: bool) {
    if (!is_dealer) {
...
#[test_only]
public fun player_won_post_handling_for_test(game: &mut Game, ctx: &mut TxContext) {
