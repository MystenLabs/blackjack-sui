...
use blackjack::single_player_blackjack::{
    Self as bj,
    HouseAdminCap,
    HouseData,
    Game,
...
fun initialize_house_data_for_test(scenario: &mut Scenario, admin: address, balance: u64) {
    scenario.next_tx(admin);
    {
...
fun initialize_game_for_test(scenario: &mut Scenario, player: address, balance: u64) {
    scenario.next_tx(player);
    {
        let coin = coin::mint_for_testing<SUI>(balance, scenario.ctx());
        let mut house_data = scenario.take_shared<HouseData>();
...
fun do_first_deal_for_test(scenario: &mut Scenario, admin: address, log_points: bool) {
...
fun do_hit_for_test(scenario: &mut Scenario, player: address, admin: address) {
    scenario.next_tx(player);
    {
        let mut game = scenario.take_shared<Game>();
        let current_player_sum = game.player_sum();
...
fun do_stand_for_test(scenario: &mut Scenario, player: address, admin: address) {
...
fun hit_for_test(scenario: &mut Scenario, admin: address) {
    // Create Random object using system address
    scenario.next_tx(@0x0);
    {
        sui::random::create_for_testing(scenario.ctx());
...
fun stand_for_test(scenario: &mut Scenario, admin: address) {
    // Create Random object using system address
    scenario.next_tx(@0x0);
    {
        sui::random::create_for_testing(scenario.ctx());
...
fun player_won_post_handling_for_test(scenario: &mut Scenario, admin: address) {
    scenario.next_tx(admin);
    {
        let mut game = scenario.take_shared<Game>();
...
fun house_won_post_handling_for_test(scenario: &mut Scenario, admin: address) {
    scenario.next_tx(admin);
    {
        let mut game = scenario.take_shared<Game>();
        let mut house_data = scenario.take_shared<HouseData>();
...
fun tie_post_handling_for_test(scenario: &mut Scenario, admin: address) {
    scenario.next_tx(admin);
    {
        let mut game = scenario.take_shared<Game>();
        let mut house_data = scenario.take_shared<HouseData>();
